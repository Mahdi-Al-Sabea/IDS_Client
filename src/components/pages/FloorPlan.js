...
export default function FloorPlan() {
  ...
  const totalFloors = 10; // ⬅️ set your actual total floors
  ...
  return (
    <div className="floorplan-container container card shadow-lg">
      <div className="card-header text-center">
        <h4>🏢 Select a Room to Reserve</h4>
      </div>
      ...
      {floorRooms.map((room) => {
        const pos = roomsPositions.find(
          (p) => parseInt(p.id) === parseInt(room.id)
        );
        if (!pos) return null; // skip if no matching position
